...
def configure_origin(value, device=None):
    ...
    matrix : (4,4) float or None
        The created matrix.
    """
    assert isinstance(value, torch.Tensor), 'Invalid type for origin, expect 4x4 torch tensor'
...
class TorchJoint(Joint):
    ...
    def _rotation_matrices(self, angles, axis):
        ...
        M[:, :3, :3] += torch.tensor([
            [0.0, -axis[2], axis[1]],
            [axis[2], 0.0, -axis[0]],
            [-axis[1], axis[0], 0.0]], device=self.device
        ).repeat((len(angles), 1, 1)) * sina[:, np.newaxis, np.newaxis]
        return M
    ...
    def get_child_poses(self, cfg, n_cfgs):
        ...
        if cfg is None:
            ...
            return torch.matmul(self.origin, self._rotation_matrices(cfg.float(), self.axis))
    ...
    def link_fk_batch(self, cfgs=None, use_names=False):
        ...
        for lnk in self._reverse_topo:
            ...
            for i in range(len(path) - 1):
                ...
                poses = torch.matmul(
                    joint.get_child_poses(cfg_vals, n_cfgs),
                    poses
                )
